...
export function Ranking() {
  ...
  // Limit display to 20 if no filters are applied to prevent huge lists, otherwise show all matches
  const displayStudents = (searchQuery || selectedStream || selectedClass) ? filteredStudents : filteredStudents.slice(0, 10);
      ...
      {loading ? (
        ...
        <div className="w-full relative">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-amber-400 focus:bg-white outline-none transition-all font-bold text-slate-700"
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-100 relative overflow-hidden">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center relative z-10">
            ...
            {(searchQuery || selectedStream || selectedClass) ? 'Keputusan Carian' : 'Top 10 Murid Paling Aktif'}
          </h2>
          
          <div className="space-y-4 relative z-10">
            {displayStudents.length === 0 ? (
              <p className="text-slate-500 italic text-center py-8">Belum ada rekod. Jadi yang pertama!</p>
            ) : (
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg mr-4 shadow-sm shrink-0
                        ${i === 0 && !searchQuery && !selectedStream && !selectedClass ? 'bg-amber-100 text-amber-700 ring-2 ring-amber-300' : 
                          i === 1 && !searchQuery && !selectedStream && !selectedClass ? 'bg-slate-200 text-slate-700 ring-2 ring-slate-300' : 
                          i === 2 && !searchQuery && !selectedStream && !selectedClass ? 'bg-orange-100 text-orange-800 ring-2 ring-orange-300' : 
                          'bg-white text-slate-500 border border-slate-200'}`}
                      >
                        {(!searchQuery && !selectedStream && !selectedClass) ? (i + 1) : '-'}
                      </div>
                      <div>
            ...
            {topClasses.length === 0 ? (
              ...
              topClasses.map((cls, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-colors border border-slate-100">
                  <div className="flex items-center">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-lg mr-4 bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100`}>
                      #{i + 1}
                    </div>
                    <div className="font-bold text-slate-900 text-xl">{cls.name}</div>
                  </div>
